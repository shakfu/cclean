...
- [x] **`--yes`**: Explicit unattended deletion without changing the default
  confirmation behavior.

- [x] **Age and size filters**: `--older-than` and `--larger-than` restrict
  candidates before confirmation; both are available in configuration.
...
- [x] **Pre-delete validation**: targets are rechecked before removal so a
  changed or replaced path is rejected.
